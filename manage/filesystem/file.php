...
<div class="file">
   ...
   $d = dir($filePhat); 
	...
	foreach($out as $k=>$v){
		if(strstr($k,".")){
    		echo   "<li><span style='float:left'> <a title='打开' target='_blank'  href= '".$filePhat."/".$k."' > $k </a> </span><span style='float:right'> <a href=javascript:if(confirm('删除操作不可恢复，确定要执行此操作吗？')){location.href='file.php?file=".$nowFilePath."/".$k."&action=delfile'} >删除</a></span></li>";
		}else{
			echo   " <p><a title='打开' href= 'file.php?file=".$nowFilePath."/".$k."' ><img src='../images/fieldir.gif' /></a><br> $k  <br><a href=javascript:if(confirm('删除操作不可恢复，确定要执行此操作吗？')){location.href='file.php?file=".$nowFilePath."/".$k."&action=del'} >删除</a> <p>";
		}
	}
   ?> 
