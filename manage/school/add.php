...
<?
	if (gg("action")=="insertadd"){
		...
		$dd->closeDb();
		$url = "add.php".getSplit(getSplit(getUrl(),"&action",0),".php",1);
...
<div>
 ...
 <? if($s_content=="1"){?> 
        ...
        <? if($gb=="1"){?>
      <tr><td width="11%">学校介绍:</td><td width="89%">
           <textarea id="editor_1" name="s_content" style="width:780px;height:300px;"><?=gv("s_content")?></textarea>
          </td>
       </tr>
       <? }?>

        <tr><td width="11%">招生介绍:</td><td width="89%">
          <textarea name="s_content1" id="editor_2" style="width:780px;height:300px;"><?=gv("s_content1")?></textarea>
          </td>
       </tr>
