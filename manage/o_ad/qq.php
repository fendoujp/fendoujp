 ...
 ?>      
  <form action="<?=getSplit(getUrl(),"&action=",0)?>&action=check&id=<?=$rs["id"]?>" method="post" id="myforms">
	 <tr>
      ...
      <? if($s_text>0){?>
      <td width="10%"><input value="<?=$rs["s_name"]?>" name="s_text" type="text" id="s_text"  size="20" /></td>
      <? }?>
      <? if($s_text>1){?>
      <td width="10%"><input value="<?=$rs["s_name1"]?>" name="s_text1" type="text" id="s_text1"  size="20" /></td>
      <? }?>
      <? if($s_text>2){?>
