...
<?
	function gv($text){
		...
		$dan = new db;
		if($id!=""){
			return ($dan->get_one("p_main",$text,$id));
		}else{
			return ("");	
		}
	}
...
<?
	if (gg("action")=="insertadd"){
		...
		//$dd->dateArr["s_price"]=gp("s_price");
		//$dd->dateArr["s_price1"]=gp("s_price1");
		...
		$dd->phpUpdate("select * from p_main where id = ".$id."");
		$dd->closeDb();
		$url = "p_do.php".getSplit(getSplit(getUrl(),"&action",0),".php",1);
...
<div>
 ...
 <form  method="post" action="<?=getSplit(getUrl(),"&action",0)?>&action=insertadd" id="forms" name="forms">   
  ...
  <? }?>    
  		<? if($s_price>1){?>
 		<tr><td width="11%">价格:</td><td width="89%"><input name="s_price" id="s_price" size="30" value="<?=gv("s_price")?>"  /></td></tr>
      	<? }?>
        <? if($s_price>=2){?>
 		<tr><td width="11%">价格2:</td><td width="89%"><input name="s_price1" id="s_price1" size="30" value="<?=gv("s_price1")?>"  /></td></tr>
      	<? }?>
  <? if($s_f>=1){?>
   ...
   <? if($s_f>=7){?>     
     ...
     <? }?>
        ...
        <? if($s_img>0){?>
      	<tr><td width="11%">缩略图 :
         <img onmouseover="this.width=100;this.height=100" onmouseout="this.width=30;this.height=30" src="../../<?=gv("s_img")?>" width="30" height="30" />
         </td><td width="89%"><?=upload("s_img",gv("s_img"))?> </td></tr>
        <? }?>
        <? if(ture){?>
        <tr><td width="11%">大图:
        <img onmouseover="this.width=100;this.height=100" onmouseout="this.width=30;this.height=30" src="../../<?=gv("s_img1")?>" width="30" height="30" />
        </td><td width="89%"><?=upload("s_img1",gv("s_img1"))?></td></tr>
        <? }?>
        <? if($s_img>2){?>
        <tr><td width="11%">上传图片3:
        <img onmouseover="this.width=100;this.height=100" onmouseout="this.width=30;this.height=30" src="../../<?=gv("s_img2")?>" width="30" height="30" />
        </td><td width="89%"><?=upload("s_img2",gv("s_img2"))?></td></tr>
        <? }?>
        <? if($s_img>3){?>
        <tr><td width="11%">上传图片4:
        <img onmouseover="this.width=100;this.height=100" onmouseout="this.width=30;this.height=30" src="../../<?=gv("s_img3")?>" width="30" height="30" />
        </td><td width="89%"><?=upload("s_img3",gv("s_img3"))?></td></tr>
        <? }?>
        <? if($s_img>4){?>
        <tr><td width="11%">上传图片5:
        <img onmouseover="this.width=100;this.height=100" onmouseout="this.width=30;this.height=30" src="../../<?=gv("s_img4")?>" width="30" height="30" />
        </td><td width="89%"><?=upload("s_img4",gv("s_img4"))?></td></tr>
        <? }?>
        <? if($s_img>5){?>
        <tr><td width="11%">上传图片6:
        <img onmouseover="this.width=100;this.height=100" onmouseout="this.width=30;this.height=30" src="../../<?=gv("s_img5")?>" width="30" height="30" />
        </td><td width="89%"><?=upload("s_img5",gv("s_img5"))?></td></tr>
        <? }?>
        <? if($s_img>6){?>
        <tr><td width="11%">上传图片7:
        <img onmouseover="this.width=100;this.height=100" onmouseout="this.width=30;this.height=30" src="../../<?=gv("s_img6")?>" width="30" height="30" />
        </td><td width="89%"><?=upload("s_img6",gv("s_img6"))?></td></tr>
        <? }?>
        <? if($s_img>7){?>
        <tr><td width="11%">上传图片8:
        <img onmouseover="this.width=100;this.height=100" onmouseout="this.width=30;this.height=30" src="../../<?=gv("s_img7")?>" width="30" height="30" />
        </td><td width="89%"><?=upload("s_img7",gv("s_img7"))?></td></tr>
        <? }?>
        <? if($s_img>8){?>
        <tr><td width="11%">上传图片9:
        <img onmouseover="this.width=100;this.height=100" onmouseout="this.width=30;this.height=30" src="../../<?=gv("s_img8")?>" width="30" height="30" />
        </td><td width="89%"><?=upload("s_img8",gv("s_img8"))?></td></tr>
        <? }?>
        <? if($s_img>9){?>
        <tr><td width="11%">上传图片10:
        <img onmouseover="this.width=100;this.height=100" onmouseout="this.width=30;this.height=30" src="../../<?=gv("s_img9")?>" width="30" height="30" />
        </td><td width="89%"><?=upload("s_img9",gv("s_img9"))?></td></tr>
        <? }?>
        <? if($s_down==1){?>
 ...
 <? if($s_content=="1"){?> 
      ...
      <tr><td width="11%">详细内容:</td><td width="89%">
            <textarea id="editor_1" name="s_content" style="width:780px;height:300px;"><?=gv("s_content");?></textarea>
           <?php
   //          $s_content=gv("s_content");
			// $oFCKeditor = new FCKeditor('s_content') ;//建立对象
   //          $oFCKeditor->BasePath = '../FCKeditor/' ;//FCKeditor所在的位置
   //          $oFCKeditor->ToolbarSet = 'Default' ;//工具按钮
			// $oFCKeditor->Value = $s_content;//初始值
			// $oFCKeditor->Height='350px';  //高度
			// $oFCKeditor->Width='780px';  //宽度
   //          $oFCKeditor->Create('s_content') ;
           ?>
          </td>
       </tr>
       <? }?>
