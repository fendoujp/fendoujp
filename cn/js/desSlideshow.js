...
(function(a){
    a.fn.desSlideshow=function(p){
        ...
        var slideshow_width=p&&p.slideshow_width?p.slideshow_width:"695";
        ...
        function init(){
            ...
            g.find(".switchBigPic").children("div").children("p").css("position","absolute").css("padding","10px").css("margin","0px").css("bottom","0px").css("opacity","0.6").css("background-color","#000").css("color","#FFF").css("font-size","12px").css("width","100%");
        ...
        function showpic(){
            ...
            g.find(".nav").children("li").eq(current).css("background-image","url("+directory+"flash-on.png)").css("border-bottom","none");
            g.find(".switchBigPic").children("div").stop(true, true).fadeOut();
            g.find(".switchBigPic").children("div").eq(current).stop(true, true).fadeIn();
            g.find(".nav").children("li").eq(current).stop().animate({marginLeft: '-35px'}, "fast").children().css("color","#fff");
            g.find(".nav").children("li").eq(w).stop().animate({marginLeft: '0px'}, "fast").children().css("color","#000");
            w = current;
