...
<?
	if(gg("action")=="check"){
		...
		$dd->phpUpdate("select * from status where id = ".gg("id")."  ");
		$dd->closeDb();
	...
	if(gg("action")=="add"){
		...
		$dd->addNews = true;
		$dd->phpUpdate("select * from status   ");
		$dd->closeDb();
		$url = getSplit(getUrl(),"&action=",0);
		//ee($url);
		ee(js("location.href='".$url."'"));
	...
	if(gg("action")=="del"){
		$dd = new db();
		$dd->setQuser("delete from status where id = ".gg("id")."  ");
		$url = getSplit(getUrl(),"&action=",0);
		ee(js("location.href='".$url."'"));
	}
	...
	if(gg("action")=="checkallorder"){
		...
		for($i=0;$i<count($allidarr);$i++){
			$dd->setQuser("update status  set s_order = ".$allorderarr[$i]." where id = ".$allidarr[$i]."  ");
		}
		$url = getSplit(getUrl(),"&action=",0);
		ee(js("location.href='".$url."'"));
	}
...
<?
	...
	$result = $dan->setQuser("select DISTINCT s_type,s_shuo from  status order by s_type asc ");
...
?>  
 ...
 <?
	...
	$result = $dan->setQuser("select * from  status where 1=1  ".$sqld." order by s_type asc,id desc ");
	while($rs=$dan->setFetch($result)){
 ?>      
