...
<INPUT TYPE="hidden" name="MAX_FILE_SIZE" value="<?=($GLOBALS["nAllowSize"]*1024*100);?>"> 
...
function DoSave(){
	...
	if($_FILES['uploadfile']['error'] > 0){
		...
		case UPLOAD_ERR_FORM_SIZE: 
			OutScript("parent.UploadError('lang[\"ErrUploadSizeLimit\"]+\":".($GLOBALS["nAllowSize"]*100)."KB\"')");
			break;
		}
		exit;
	}
...
function SaveRemoteFile($s_LocalFileName, $s_RemoteFileUrl) { 
	...
	if (strlen($cont) > $GLOBALS["nAllowSize"]*1024*100) {
		return false;
	}
