...
<?
	...
	if($s_next==""){$s_next=0;}
	$dd = new db();
	$class_depth = $dd->get_s("select class_depth from a_class  where parent_id = ".$parent_id."   ");
	//e($class_depth);
?>
<?
	if(gg("action")=="checkName"){
		$da = new db;
		$da->setQuser("update a_class set s_name = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="checkNum"){
		$da = new db;
		$da->setQuser("update a_class set s_order = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="del"){
		$da = new db;
		$da->setQuser("delete from a_class  where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="pdel"){
		$da = new db;
		$da->setQuser("delete from a_class  where id in( ".gp("allids")." ) ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="checkok"){
		$da = new db;
		$nowNum = $da->getCount("select * from a_class where s_type = '".$s_type."' and s_ok = ".unescape(gg("value"))."   ");
		...
		$da->setQuser("update a_class set s_ok = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	...
	if(gg("action")=="checkallorder"){
		...
		for($i=0;$i<count($allidarr);$i++){
			$dd->setQuser("update a_class  set s_order = ".$allorderarr[$i]." where id = ".$allidarr[$i]."  ");
		}
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
...
<option value="">默认</option>
    ...
    &nbsp;&nbsp;&nbsp;
    <? if($s_add=="1"){?>
    <input onclick="location.href='a_class_detail.php<?=getSplit(getSplit(getUrl(),".php",1),"&id=",0)?>'" value="添加本级分类" type="button" />
    <? }?>
    &nbsp;&nbsp;&nbsp;&nbsp;
...
<?
	...
	$sqlStr = "";
	$pp = new page(gg("pageId"),20,$dan->getCount("select * from a_class  where 1=1 and parent_id=".$parent_id." and s_type = '".$s_type."'  ".$sqld."   "),getUrl());
	$result = $dan->setQuser("select * from a_class  where 1=1 and parent_id=".$parent_id." and s_type = '".$s_type."'  ".$sqld."   limit ".$pp->limitStart().",".$pp->limitEnd()."  ");
	$ii=0;
...
?>       
 ...
 <? if($s_del=="1"){?> 
 <td width="3%">
 <? if($dan->getCount("select * from a_class where 1=1 and parent_id= ".$rs["id"]."  ")=="0"){
	 	if($dan->getCount("select * from a_main where 1=1 and classid = ".$rs["id"]."   ")==""){
?>
...
<? if($s_check=="1"){?>
 ...
 <? if($s_next!="0"){?>
 	<? if($class_depth<$s_next){?>
 <td width="23%"><a href="<?=getSplit(getUrl(),"&parent_id=",0)?>&parent_id=<?=$rs["id"]?>">二级分类(<?=$dan->getCount("select * from a_class where 1=1 and parent_id= ".$rs["id"]."  ")?>)</a></td>
 	<? }?>
 <? }?>
...
<? if($s_oks=="1"){?>
 ...
 <?// if($s_check=="1"){?>
 <input onclick="location.href='a_class_detail.php<?=getSplit(getSplit(getUrl(),".php",1),"&id=",0)?>&id=<?=$rs["id"]?>'" type="button" value="修改" />
 <?//   }?>
 <? if($s_del=="1"){?>
 &nbsp;&nbsp;&nbsp;&nbsp;
 <? if($dan->getCount("select * from a_class where 1=1 and parent_id= ".$rs["id"]."  ")=="0"){
