...
<?
	if(gg("action")=="checkName"){
		$da = new db;
		$da->setQuser("update info set s_name = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="checkNum"){
		$da = new db;
		$da->setQuser("update info set s_order = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="del"){
		$da = new db;
		$da->setQuser("delete from info  where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="pdel"){
		$da = new db;
		$da->setQuser("delete from info  where id in( ".gp("allids")." ) ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="checkok"){
		$da = new db;
		$nowNum = $da->getCount("select * from info where s_type = '".$s_type."' and s_ok = ".unescape(gg("value"))."   ");
		...
		$da->setQuser("update info set s_ok = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
  ...
  if(gg("action")=="checkclassid"){
    $da = new db;
    $da->setQuser("update info set classid = '".unescape(gg("value"))."' where id = ".gg("id")." ");
    $da->closeDb();
    ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
  }
	...
	if(gg("action")=="attri"){
		$da = new db;
		$sok2allstr="";
		$sok2all = getLiAll("select s_ok2 from info  where s_type = '".$s_type."'  ");
		...
		$da->setQuser("update info set s_ok2 = '".unescape(gg("value"))."' where id = ".gg("infoid")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	...
	if(gg("action")=="checkallorder"){
		...
		for($i=0;$i<count($allidarr);$i++){
			$dd->setQuser("update info  set s_order = ".$allorderarr[$i]." where id = ".$allidarr[$i]."  ");
		}
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
...
<option value="">默认</option>
    ...
    &nbsp;&nbsp;&nbsp;
    <? if($s_add=="1"){?>
    <input onclick="location.href='info_detail.php<?=getSplit(getSplit(getUrl(),".php",1),"&id=",0)?>'" value="添加<?=$s_title?>" type="button" />
    <? }?>
...
<?
	...
	$sqlStr = "";
	$pp = new page(gg("pageId"),20,$dan->getCount("select * from info  where 1=1 and s_type = '".$s_type."'  ".$sqld."   "),getUrl());
	$result = $dan->setQuser("select * from info  where 1=1 and s_type = '".$s_type."'  ".$sqld."   limit ".$pp->limitStart().",".$pp->limitEnd()."  ");
	$ii=0;
...
<? if($s_oks2=="1"){?>
 ...
 <?// if($s_check=="1"){?>
 <input onclick="location.href='info_detail.php<?=getSplit(getSplit(getUrl(),".php",1),"&id=",0)?>&id=<?=$rs["id"]?>'" type="button" value="修改" />
 <?//   }?>
 <? if($s_del=="1"){?>
 &nbsp;&nbsp;&nbsp;&nbsp;
