...
<?
	if(gg("action")=="checkName"){
		$da = new db;
		$da->setQuser("update main_info set s_name = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="checkNum"){
		$da = new db;
		$da->setQuser("update main_info set s_order = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="del"){
		$da = new db;
		$da->setQuser("delete from main_info  where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="pdel"){
		$da = new db;
		$da->setQuser("delete from main_info  where id in( ".gp("allids")." ) ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="checkok"){
		$da = new db;
		$nowNum = $da->getCount("select * from main_info where s_type = '".$s_type."' and s_ok = ".unescape(gg("value"))."   ");
		...
		$da->setQuser("update main_info set s_ok = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
...
<option value="">默认</option>
    ...
    &nbsp;&nbsp;&nbsp;
    <? if($s_add=="1"){?>
    <input onclick="location.href='main_info_detail.php<?=getSplit(getSplit(getUrl(),".php",1),"&id=",0)?>'" value="添加<?=$s_title?>" type="button" />
    <? }?>
...
<?
	...
	$sqlStr = "";
	$pp = new page(gg("pageId"),20,$dan->getCount("select * from main_info  where 1=1 and s_type = '".$s_type."'  and s_classtype = '".$s_classtype."'  and classid = ".$classid." ".$sqld."   "),getUrl());
	$result = $dan->setQuser("select * from main_info  where 1=1 and s_type = '".$s_type."'  and s_classtype = '".$s_classtype."' and classid = ".$classid."  ".$sqld."   limit ".$pp->limitStart().",".$pp->limitEnd()."  ");
	$ii=0;
...
<? if($s_check=="1"){?>
 ...
 <td width="12%"><?=$rs["s_time"]?></td>
 <td width="5%"><input onchange="location.href='<?=getUrl()?>&action=checkNum&id=<?=$rs["id"]?>&value='+escape(this.value)" value="<?=$rs["s_order"]?>" size="10" /></td>
<? $cc = new db;?>
<? if($s_oks=="1"){?>
 <td width="5%">
 ...
 <?// if($s_check=="1"){?>
 <input onclick="location.href='main_info_detail.php<?=getSplit(getSplit(getUrl(),".php",1),"&id=",0)?>&id=<?=$rs["id"]?>'" type="button" value="修改" />
 <?//   }?>
 <? if($s_del=="1"){?>
 &nbsp;&nbsp;&nbsp;&nbsp;
...
<? $ii++;  }?>            
      ...
      <tr>
        <td colspan="17" class="page" style="height:25px; padding:5px 0; "> 
