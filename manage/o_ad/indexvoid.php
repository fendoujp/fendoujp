...
<?
	...
	$id = gg("id");
	$dis = gg("disabled");
	if($dis==0){$dis="";}else{$dis="disabled";}
?>
<?
	function gpimg($text){
 ...
 ?>      
  <form action="<?=getSplit(getUrl(),"&action=",0)?>&action=check&id=<?=$rs["id"]?>" method="post" id="myforms">
	 <tr>
      ...
      <? if($s_text>0){?>
      <td width="10%">
      <input <?=$dis?> value="<?=$rs["s_name"]?>" name="s_text" type="text" id="s_text"  size="30" />
      </td>
      <? }?>
      <? if($s_text>1){?>
      <td width="10%"><input value="<?=$rs["s_name1"]?>" name="s_text1" type="text" id="s_text1"  size="30" /></td>
      <? }?>
      <? if($s_text>2){?>
     ...
     <? if($s_img>0){?>
	  ...
	  <?=upload("s_img".$rs["id"],$rs["s_img"])?></td>
      <? }?>
     <? if($s_img>1){?>
