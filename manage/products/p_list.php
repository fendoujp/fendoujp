...
<?
	if(gg("action")=="checkName"){
		$da = new db;
		$da->setQuser("update p_main set s_name = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="checkNum"){
		$da = new db;
		$da->setQuser("update p_main set s_order = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	if(gg("action")=="del"){
		$da = new db;
		$da->setQuser("delete from p_main  where id = ".gg("id")." ");
	...
	if(gg("action")=="pdel"){
		$da = new db;
		$da->setQuser("delete from p_main  where id in( ".gp("allids")." ) ");
	...
	if(gg("action")=="checkok"){
		$da = new db;
		$nowNum = $da->getCount("select * from p_main where s_type = '".$s_type."' and s_ok = ".unescape(gg("value"))."   ");
		...
		$da->setQuser("update p_main set s_ok = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	...
	if(gg("action")=="attri"){
		$da = new db;
		$sok2allstr="";
		$sok2all = getLiAll("select s_ok2 from p_main  where s_type = '".$s_type."'  ");
		...
		$da->setQuser("update p_main set s_ok2 = '".unescape(gg("value"))."' where id = ".gg("infoid")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	...
	if(gg("action")=="checkclassid"){
		$da = new db;
		$da->setQuser("update p_main set classid = '".unescape(gg("value"))."' where id = ".gg("id")." ");
		$da->closeDb();
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
	...
	if(gg("action")=="checkallorder"){
		...
		for($i=0;$i<count($allidarr);$i++){
			$dd->setQuser("update p_main  set s_order = ".$allorderarr[$i]." where id = ".$allidarr[$i]."  ");
		}
		ee(js("location.href='".getSplit(getUrl(),"&action",0)."'"));
	}
...
<option value="">默认</option>
    ...
    &nbsp;&nbsp;&nbsp;
    <? if($s_add=="1"){?>
    <input onclick="location.href='p_do.php<?=getSplit(getSplit(getUrl(),".php",1),"&id=",0)?>'" value="添加" type="button" />
    <? }?>
...
<?
	...
	if($ordertype=="s_order"){
		 $sqld = $sqld."order by s_order ";
		 }
	...
	$sqlStr = "";
	$pp = new page(gg("pageId"),20,$dan->getCount("select * from p_main  where 1=1 and s_type = '".$s_type."'  ".$sqld."   "),getUrl());
	$result = $dan->setQuser("select * from p_main  where 1=1 and s_type = '".$s_type."'  ".$sqld."   limit ".$pp->limitStart().",".$pp->limitEnd()."  ");
	$ii=0;
...
<? if($s_oks=="9"){?>
 ...
 <?// if($s_check=="1"){?>
 <input onclick="location.href='p_do.php<?=getSplit(getSplit(getUrl(),".php",1),"&id=",0)?>&id=<?=$rs["id"]?>'" type="button" value="修改" />
 <?//   }?>
<? if($s_xx=="1"){?> 
 &nbsp;&nbsp; 
 <input onclick="location.href='../info/main_info.php?s_type=p_maininfo&s_classtype=product&classid=<?=$rs["id"]?>&s_img=1&s_add=1&s_del=1&s_check=1&s_ok=1&s_down=1&s_conj=0&s_content=1&title=详细参数'" type="button" value="详细" />
 <? }?>
 <?php if(false){?>
 &nbsp;&nbsp; 
 <input onclick="location.href='../info/main_info.php?s_type=p_maininfo&s_classtype=product&classid=<?=$rs["id"]?>&s_img=1&s_add=1&s_del=1&s_check=1&s_ok=0&s_down=1&s_conj=0&s_content=0&title=多角度展'" type="button" value="多角度展示" />
  <?php }?>
 <? if($s_del=="1"){?>
 &nbsp;&nbsp; 
