...
<?
	function gv($text){
		...
		$dan = new db;
		if($id!=""){
			return ($dan->get_one("a_class",$text,$id));
		}else{
			return ("");	
		}
	}
...
<?
	...
	//ee($parent_id);
	$dd = new db();
	$class_depth=$dd->get_s("select class_depth from a_class where id = ".$parent_id." ");
...
<?
	if (gg("action")=="insertadd"){
		...
		$dd->phpUpdate("select * from a_class where id = ".$id."");
		$dd->closeDb();
		$url = "a_class.php".getSplit(getSplit(getSplit(getUrl(),"&action",0),".php",1),"&id=",0);
