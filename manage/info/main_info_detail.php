...
<?
	function gv($text){
		...
		$dan = new db;
		if($id!=""){
			return ($dan->get_one("main_info",$text,$id));
		}else{
			return ("");	
		}
	}
...
<?
	if (gg("action")=="insertadd"){
		...
		$dd->dateArr["s_classtype"]=gg("s_classtype");
		$dd->dateArr["classid"]=gg("classid");
		...
		$dd->phpUpdate("select * from main_info where id = ".$id."");
		$dd->closeDb();
		$url = "main_info.php".getSplit(getSplit(getSplit(getUrl(),"&action",0),".php",1),"&id=",0);
