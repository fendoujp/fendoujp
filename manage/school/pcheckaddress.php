...
<?
	...
	if($action=="checkall"){
		...
		for($i=0;$i<count($idsArr);$i++){		
			...
			$dd->phpUpdate("select * from a_main where id = ".$idsArr[$i]."");	
		}
    ...
    <form method="post" id="myforms" name="myforms" action="?action=checkall">
     ...
     <? for($i=0;$i<count($idsArr);$i++){?> 
		<tr>
        	<td width="5%">名称</td>
        	<td width="27%">
				<input name="s_name<?=$idsArr[$i]?>" type="text" id="s_name<?=$idsArr[$i]?>" value="<?=get_o("a_main","s_name",$idsArr[$i])?>" size="50" />
            ...
            <td width="35%">
                ...
                <?php foreach($list_arr as $k=>$v){?> 
                  <option value="<?php echo $v['id'];?>"<?php if($v['id']==get_o("a_main","classid",$idsArr[$i]))echo' selected="selected"';?>>├<?php for($s=1;$s<$v['class_depth'];$s++){echo'─';}?><?php echo $v['s_name'];?></option>
                <?php }?>
                </select>
