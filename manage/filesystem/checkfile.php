...
<?
	if(gg("myselfname")!=""){
		$myselfname = gg("myselfname");
		$_SESSION["xingmyselfname"]=$myselfname;
	}
?>
<?php
...
<div class="file">
   ...
   $d = dir($filePhat); 
	...
	foreach($out as $k=>$v){
		if(strstr($k,".")){
			//".$_SESSION["xingmyselfname"]."
    		echo   "<li><span style='float:left'> <a title='打开' target='_blank'   href= '".$filePhat."/".$k."' > $k </a> </span><span style='float:right'> <a href=javascript:func1('../../uploadfile".$nowFilePath."/".$k."'); >选择</a></span></li>";
		}else{
			echo   " <p><a  title='打开' href= 'checkfile.php?file=".$nowFilePath."/".$k."' ><img src='../images/fieldir.gif' /></a><br> $k  <br><a href=javascript:if(confirm('删除操作不可恢复，确定要执行此操作吗？')){location.href='checkfile.php?file=".$nowFilePath."/".$k."&action=del'} ></a> <p>";
		}
	}
   ?> 
