...
<?
	...
	if(gg("action")=="add"){
		...
		ee(js("location.href='menu_class.php?parent_id=".gg("parent_id")."'"));
	}
	if(gg("action")=="del"){
		delMenu(gg("id"));
		ee(js("location.href='menu_class.php?parent_id=".gg("parent_id")."'"));
...
<div class="title">栏目管理</div>
  ...
  <div class="bacf1 borddd marbot10 pad5">
    <?=getSelect($parent_id,"s_name","select * from s_menu_class where 1=1 and s_ok=1 and parent_id=0 order by s_order asc,id desc")?>
    </div>
  ...
  <form action="?action=checkall&id=<?=$rs["id"]?>&parent_id=<?=$parent_id?>" method="post" id="myforms">   
      ...
      <td>
		 <?=getSelect2($parent_id,$rs["id"],"s_name","select * from s_menu_class where 1=1 and s_ok=1 and parent_id=0 order by s_order asc,id desc")?>
      </td>
      <td><input value="<?=$rs["s_name"]?>" name="s_name" id="s_name" size="10"  /></td>
      <td class="menulistk">
      <input value="<?=$rs["s_url"]?>" name="s_url" id="s_url<?=$rs["id"]?>" size="80"  />
      ...
      <a href="javascript:void(0);" onclick="menuF('menulistn<?=$rs["id"]?>');">编辑</a>
      <div style="display:none;" class="menulistn" id="menulistn<?=$rs["id"]?>">
      	<p class="menulistpm"><a href="javascript:" onclick="menuF('menulistn<?=$rs["id"]?>');">关闭</a></p>
        
         <p class="menulistp"><input name="menurequest" style="width:200px;" value="<?=getSplit($rs["s_url"],"?",0)?>" /></p>
      <?
	  if(strstr($rs["s_url"],"?")){
		$geturlarr =  splitArray(getSplit($rs["s_url"],"?",1),"&");
		for($iii=0;$iii<count($geturlarr);$iii++){
	?>
 	 <p class="menulistp"> <input name="menurequest" style="width:200px;" value="<?=$geturlarr[$iii]?>" /></p>
	<? }}?>
      
      <p class="menulistp">
      <input onclick="getLvalue('menulistn<?=$rs["id"]?>','s_url<?=$rs["id"]?>')" value="确定" type="button" />&nbsp;&nbsp;&nbsp;
      <input value="取消" type="button" onclick="menuF('menulistn<?=$rs["id"]?>');" />
      </p>
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;  <input value="批量修改排序" type="button" onclick="psave();" />
    ...
    <form style="display:none" action="?action=checkallorder&parent_id=<?=$parent_id?>" method="post" id="yforms">
     ...
     <form style="display:none" action="?action=checkallokm&parent_id=<?=$parent_id?>" method="post" id="yformsm">
...
function getLvalue(id,id2){
	var str="";
	var mms = document.getElementById(id).getElementsByTagName("input");
	//document.getElementsByName(
	for(var i=0;i<mms.length;i++){
		if(mms.item(i).value!="确定"&&mms.item(i).value!="取消"){
			if(i==0){
				if(mms.length>3){
				str = str +mms.item(i).value+"?";
				}else{
					str = str +mms.item(i).value;
				}
			}
