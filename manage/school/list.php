...
<option value="">默认</option>
    ...
    &nbsp;&nbsp;&nbsp;
    <? if($s_add=="1"){?>
    <input onclick="location.href='add.php<?=getSplit(getSplit(getUrl(),".php",1),"&id=",0)?>'" value="添加" type="button" />
    <? }?>
...
<? if($s_oks=="9"){?>
 ...
 <?// if($s_check=="1"){?>
 <input onclick="location.href='add.php<?=getSplit(getSplit(getUrl(),".php",1),"&id=",0)?>&id=<?=$rs["id"]?>'" type="button" value="修改" />
 <?//   }?>
  &nbsp;&nbsp; 
 <input onclick="location.href='../info/main_info.php?s_type=school&s_classtype=school&classid=<?=$rs["id"]?>&s_img=1&s_add=1&s_del=1&s_check=1&s_ok=0&s_down=1&s_conj=0&s_content=0&title=校园照片'" type="button" value="校园照片" />
 <? if($s_del=="1"){?>
 &nbsp;&nbsp;
