...
<?
	function gv($text){
		...
		$dan = new db;
		if($id!=""){
			return ($dan->get_one("info",$text,$id));
		}else{
			return ("");	
		}
	}
...
<?
	if (gg("action")=="insertadd"){
		...
		$dd->phpUpdate("select * from info where id = ".$id."");
		$dd->closeDb();
		$url = "info.php".getSplit(getSplit(getSplit(getUrl(),"&action",0),".php",1),"&id=",0);
 ...
 <form  method="post" action="<?=getSplit(getUrl(),"&action",0)?>&action=insertadd" id="forms" name="forms">   
  ...
  <? }?>    
   ...
   <? //if($s_conj>="1"){?>     
        ...
        <tr><td width="11%"><? if($s_type=='contact'){echo '地图';} else{echo '简要说明';}?>:</td><td width="89%">
          
           <textarea id="editor_2" name="s_conj" style="width:780px;height:300px;">
<?=gv("s_conj")?>
</textarea>
          </td>
       </tr>
       <? // }?>
        ...
        <tr><td width="11%">颜色:</td><td width="89%">
        
          <input name="s_conj1" id="s_conj1" size="30" value="<?=gv("s_conj1")?>"  /></td>
       </tr>
       <? }//}?>
  ...
  <? }?>  
      ...
      <tr><td width="11%">详细内容:</td><td width="89%">
          ...
          <textarea id="editor_1" name="s_content" style="width:780px;height:350px;">
<?=gv("s_content")?>
</textarea>
          </td>
       </tr>
